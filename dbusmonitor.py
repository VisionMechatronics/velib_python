...
class DbusMonitor(object):
	...
	def scan_dbus_service_inner(self, serviceName):
		...
		for path, options in paths.iteritems():
			...
			# Try to obtain the value we want from our bulk fetch. If the
			# bulk fetch failed, values and texts will evaluate as false,
			# and we need to do an individual query.
			if values and texts:
